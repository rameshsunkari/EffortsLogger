package com.efforts.utilities;

public class NavigationOutCome {

	public static final String SUCCESS = "success";
	public static final String FAILURE = "failure";
	public static final String LOGOUT = "logout";
	public static final String MY_ACCOUNT = "myAccount";
	public static final String VIEW_EFFORTS = "viewEfforts";
	public static final String LOG_EFFORTS = "logEfforts";
	public static final String VIEW_MEMBERS = "viewMembers";
	public static final String VIEW_ROLES = "viewRoles";
	public static final String VIEW_PROJECTS = "viewProjects";
	public static final String ADD_PROJECTS = "addProjects";
	public static final String ADD_ROLES = "addRoles";
	public static final String ADD_MEMBERS = "addMembers";
	public static final String ADD_EFFORTS = "addEfforts";
	public static final String VIEW_EFFORTS_TYPE = "viewEffortsType";
	public static final String VIEW_EFFORTS_SUB_TYPE = "viewEffortsSubType";
	public static final String ADD_EFFORTS_TYPE = "addEffortsType";
	public static final String ADD_EFFORTS_SUB_TYPE = "addEffortsSubType";
}
